...
openHealth.tcga = (function (oh) {
    ...
    tcga.text2table = function (x, La, Lv) {
        ...
        if (xx.slice(-1)[0].length == 1 && xx.slice(-1)[0][0].length == 0) {
            xx.pop(-1);
        } // last row is empty
        var n = xx.length,
            m = xx[0].length;
        // get attr names from second row
        var y = {};
        if (!La) {
            La = 0;
        } // default row with attribute names is the 1st
        if (!Lv) {
            Lv = 1;
        } // default row with attribute names is the 2nd
    ...
    tcga.sortPercent = function (x) { // sort percent values
        return openHealth.sort(x.map(function (x) {
            if (x == "[Not Available]") {
                return -1;
            } else {
                return parseInt(x);
